...
from acestep.constants import DEFAULT_LM_INSTRUCTION, DEFAULT_LM_UNDERSTAND_INSTRUCTION, DEFAULT_LM_INSPIRED_INSTRUCTION, DEFAULT_LM_REWRITE_INSTRUCTION, DEFAULT_LM_WRITE_FROM_SCRATCH_INSTRUCTION, DEFAULT_LM_CONDUCTOR_NOTES_INSTRUCTION, DURATION_MIN, DURATION_MAX
...
class LLMHandler:
    ...
    def generate_with_stop_condition(
        ...
        if infer_type not in {"dit", "llm_dit"}:
            ...
            return {
                ...
                "success": False,
                "error": error_msg,
                "extra_outputs": {"time_costs": {}, "phase1_reasoning": None},
            }
        ...
        if not has_all_metas and use_cot_metas:
            ...
            formatted_prompt = self.build_formatted_prompt(caption, lyrics, generation_phase="cot", system_instruction=system_instruction)
            ...
            if not cot_output_text:
                return {
                    ...
                    "success": False,
                    "error": status,
                    "extra_outputs": {"time_costs": {"phase1_time": phase1_time}, "phase1_reasoning": None},
                }
            ...
            # Extract raw CoT reasoning text for UI display (same patterns as parse_lm_output)
            import re as _re
            _reasoning_patterns = [
            ...
            for _pat in _reasoning_patterns:
                _m = _re.search(_pat, cot_output_text, _re.DOTALL)
                if _m:
                    phase1_reasoning = _m.group(1).strip()
                    break

            # Fallback: extract everything before metadata lines or audio codes
            if not phase1_reasoning:
                _fallback = cot_output_text.split('<|audio_code_')[0]
                if _fallback.strip():
                    phase1_reasoning = _fallback.strip()
    ...
    def build_formatted_prompt(self, caption: str, lyrics: str = "", is_negative_prompt: bool = False, generation_phase: str = "cot", negative_prompt: str = "NO USER INPUT", system_instruction: Optional[str] = None) -> str:
        """
        ...
        Args:
            ...
            system_instruction: Optional custom system instruction. Defaults to DEFAULT_LM_INSTRUCTION.
        ...
        instr = system_instruction or DEFAULT_LM_INSTRUCTION
    ...
    def build_formatted_prompt_for_format(
        ...
        Args:
            ...
            is_negative_prompt: If True, builds unconditional prompt for CFG
            negative_prompt: Negative prompt for CFG (used when is_negative_prompt=True)
            mode: One of 'enhance' (default), 'write_from_scratch', or 'conductor_notes'.
                  'write_from_scratch': generate lyrics from caption only.
                  'conductor_notes': generate structural arrangement tags without lyrics.
        ...
        if is_negative_prompt:
            # For CFG unconditional prompt
            user_content = negative_prompt if negative_prompt and negative_prompt.strip() else ""
        elif mode == "write_from_scratch":
            # Write from scratch: caption is the creative guidance, no lyrics to format
            user_content = f"# Caption\n{caption}"
        elif mode == "conductor_notes":
            # Conductor notes: structural arrangement guide only, no lyrics
            user_content = f"# Caption\n{caption}"
        else:
            # Normal prompt: caption + lyrics
            user_content = f"# Caption\n{caption}\n\n# Lyric\n{lyrics}"

        if mode == "write_from_scratch":
            system_instruction = DEFAULT_LM_WRITE_FROM_SCRATCH_INSTRUCTION
        elif mode == "conductor_notes":
            system_instruction = DEFAULT_LM_CONDUCTOR_NOTES_INSTRUCTION
        else:
            system_instruction = DEFAULT_LM_REWRITE_INSTRUCTION
    ...
    def format_sample_from_input(
        ...
        Args:
            ...
            use_constrained_decoding: Whether to use FSM-based constrained decoding
            constrained_decoding_debug: Whether to enable debug logging
            cfg_scale: CFG scale for generation (default 2.0). Higher = more faithful to prompt.
        ...
        # Detect mode based on lyrics content:
        # 1) Blank → write_from_scratch (write lyrics from scratch)
        # 2) Only [Instrumental] → conductor_notes (structural arrangement, no lyrics)
        # 3) Actual content → enhance (enhance both + add conductor tags)
        _mode = "enhance"
        if not lyrics or not lyrics.strip():
            _mode = "write_from_scratch"
        elif re.fullmatch(r"\s*\[instrumental\]\s*", lyrics, re.IGNORECASE):
            _mode = "conductor_notes"

        if not caption or not caption.strip():
            caption = "NO USER INPUT"

        logger.info(f"Formatting sample from input: caption={caption[:50]}..., mode={_mode}")
        ...
        # For format mode, always provide at least a language constraint so the model
        # doesn't produce garbled output in random languages when no user metadata given.
        ...
        # If no language constraint was provided, default to English so the model
        # produces readable text output instead of garbled multi-language noise.
        if constrained_metadata is None or 'language' not in constrained_metadata:
            if constrained_metadata is None:
                constrained_metadata = {}
            constrained_metadata['language'] = 'en'

        if constrained_metadata:
            logger.info(f"Using metadata constraints for format mode: {constrained_metadata}")
