...
GENERATION_MODES_TURBO = ["Simple", "Custom", "Remix", "Edit"]

# Extended modes for pure base models only — adds Extract/Lego/Complete
GENERATION_MODES_BASE = ["Simple", "Custom", "Remix", "Edit", "Extract", "Lego", "Complete"]
...
DEFAULT_LM_INSPIRED_INSTRUCTION = (
    "Generate an entirely NEW musical description inspired by the user's input. "
    "Do NOT reuse phrases from the original — create different instrumentation, mood, "
    "structure, and imagery. Think of it as a creative reinterpretation, not a paraphrase."
)
DEFAULT_LM_REWRITE_INSTRUCTION = (
    "Format the user's input into a more detailed and specific musical description. "
    "Enhance the lyrics and add structural section tags like [Verse], [Chorus], etc."
)
DEFAULT_LM_WRITE_FROM_SCRATCH_INSTRUCTION = (
    "Write complete song lyrics for a song matching this caption description. "
    "Include full structure with section tags like [Verse], [Chorus], [Bridge], etc."
)
DEFAULT_LM_CONDUCTOR_NOTES_INSTRUCTION = (
    "Create conductor notes (structural arrangement guide) for an instrumental song "
    "matching this caption description. In the lyrics field, write ONLY structural "
    "section tags with instrument arrangement cues like [Intro], [Verse], [Chorus], "
    "[Solo], [Breakdown] — do NOT write vocal lyrics. Each section tag should be "
    "followed by a brief note about which instruments play there."
)

# Inspiration mode: preset system prompts that control how the LLM transforms
# the user's caption before passing it to the DiT model. Each key maps to a
# different reasoning strategy.
INSPIRATION_PRESETS = {
    "detailed": DEFAULT_LM_INSPIRED_INSTRUCTION,
    "lyrics": (
        "Write complete song lyrics for a song matching this caption description. "
        "Include full structure with section tags like [Verse], [Chorus], [Bridge], etc."
    ),
    "conductor": (
        "Create conductor notes (structural arrangement guide) for an instrumental song "
        "matching this caption description. Write ONLY structural section tags with "
        "instrument arrangement cues — do NOT write vocal lyrics."
    ),
    "mood": (
        "Describe the mood, atmosphere, instrumentation, and production style of the "
        "music in vivid detail. Focus on sonic texture, dynamics, and emotional arc.",
    ),
}
...
TASK_INSTRUCTIONS = {
    ...
    "inspiration": "Generate new music inspired by the source audio: ",
