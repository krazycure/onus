...
from acestep.constants import BPM_MIN, BPM_MAX, DURATION_MAX, TASK_TYPES, VALID_TIME_SIGNATURES, DEFAULT_LM_INSPIRED_INSTRUCTION, INSPIRATION_PRESETS
...
class GenerationParams:
    ...
    # Inspiration mode preset (selects LLM system instruction)
    inspiration_preset: Optional[str] = None  # "detailed", "lyrics", "conductor", "mood"
...
def generate_music(
    ...
    try:
        ...
        # "inspiration" is intentionally excluded — it needs LLM to expand the prompt into rich conditioning.
        ...
        if use_lm:
            ...
            # Pick LLM system instruction based on task type (for CoT reasoning)
            lm_system_instruction = None  # defaults to DEFAULT_LM_INSTRUCTION in handler
            if params.task_type == "inspiration":
                preset_key = params.inspiration_preset or "detailed"
                lm_system_instruction = INSPIRATION_PRESETS.get(preset_key, DEFAULT_LM_INSPIRED_INSTRUCTION)
            ...
            for chunk_idx in range(num_chunks):
                ...
                result = llm_handler.generate_with_stop_condition(
                    ...
                    seeds=chunk_seeds,
                    progress=progress,
                    system_instruction=lm_system_instruction,
                )
                ...
                # Collect Phase 1 CoT reasoning (first non-null value wins)
                if "phase1_reasoning" not in locals() or phase1_reasoning is None:
                    _reasoning = lm_extra.get("phase1_reasoning")
                    if _reasoning:
                        phase1_reasoning = _reasoning
        ...
        if phase1_reasoning:
            extra_outputs["phase1_reasoning"] = phase1_reasoning
